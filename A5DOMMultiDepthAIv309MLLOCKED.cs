...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    public static class BurstTypes { public const byte NONE=0; public const byte SAME_EXCH_TS=1; public const byte PRICE_CLUSTER=2; }
        ...
        protected override void OnMarketDepth(MarketDepthEventArgs e)
        {
            try
            {
                ...
                lock (d.Sync)
                {
                    ...
                    {
                        EmitRow(d, e.Time, Tags.DOM, 0, e.Price, LiqEvents.NONE, CausalLabels.NONE, force: true, forceReason: Reasons.TIME_REGRESSION);
                        return;
                    }
                    ...
                    {
                        d.OfiCnt = 0; d.OfiMean = 0; d.OfiVar = 0; d.OfiStatResetTs = d.NextHostUtc();
                    }
                    ...
                    lock (d.HistoryLock)
                    {
                        ...
                        {
                            Seq = d.Seq, ExchTs = e.Time, HostUtc = d.NextHostUtc(),
                    ...
                    if (UseSessionFilter && !sessionIterators[idx].IsInSession(d.NextHostUtc(), true, true))
                    {
                    ...
                    // Burst logic and Feature Update logic MOVED to EmitRow
        ...
        private void EmitRow(DomState d, DateTime eventTime, byte tag, long tradeVol, double eventPrice, byte liquidityEvent, byte tradeCausalityLabel = CausalLabels.NONE, bool force = false, byte forceReason = Reasons.NONE, int liqSide = 0, int liqLevel = -1, long liqDelta = 0, bool? trainEligibleOverride = null, long sumBidK = 0, long sumAskK = 0, long currentOfi = 0, long currentDepthOfi = 0)
        {
            ...
            // [FIX] SPOOF / ICEBERG / FEATURE STATE MOVED INSIDE EMIT ROW
            if (tag == Tags.DOM)
            {
                 if (EnableSpoof && !force)
                    UpdateSpoof(d, nowUtc, liqSide == 1, eventPrice, liqDelta, d.LastBidPx, d.LastAskPx);

                 if (EnableIceberg && !force)
                    UpdateIcebergFromDepth(d, nowUtc, liqSide, eventPrice, liqDelta);

                 if (!d.InBurst && !force)
                 {
                    UpdateFeatureState(d, nowUtc, d.LastBidPx, d.LastAskPx, d.LastBidSize, d.LastAskSize, sumBidK, sumAskK, currentOfi, currentDepthOfi);
                 }
                 else if (d.InBurst)
                 {
                        d.RvVarEma = 0;
            ...
            if (tag == Tags.TRADE && !d.IsDomBootstrapped)
            {
                ...
                if (d.LastDepthUpdateExchTs > DateTime.MinValue)
                {
                    EmitRow(d, d.LastDepthUpdateExchTs, Tags.DOM, 0, 0, LiqEvents.NONE, CausalLabels.NONE, force: true, forceReason: Reasons.PRE_TRADE_SYNC);
