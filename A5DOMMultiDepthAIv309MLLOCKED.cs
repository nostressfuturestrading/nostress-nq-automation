...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    public static class BurstTypes { public const byte NONE=0; public const byte SAME_EXCH_TS=1; public const byte PRICE_CLUSTER=2; }
        ...
        // CME trading day boundary for ES/NQ: 17:00 CT
        private const int CmeTradingDayStartHourCT = 17;
        private const int CmeTradingDayStartMinuteCT = 0;

        // Defined here to support Patch A
        private static readonly TimeZoneInfo ExchangeTimeZone = TimeZoneInfo.FindSystemTimeZoneById("Central Standard Time");

        private static DateTime GetCmeTradingDayLabelCme(DateTime nowUtc)
        {
            DateTime cme = TimeZoneInfo.ConvertTimeFromUtc(nowUtc, ExchangeTimeZone);
            TimeSpan boundary = new TimeSpan(CmeTradingDayStartHourCT, CmeTradingDayStartMinuteCT, 0);

            // On/after 17:00 CT -> label next date
            if (cme.TimeOfDay >= boundary)
                return cme.Date.AddDays(1);

            return cme.Date;
        }

        private static string GetCmeTradingDayKey(DateTime nowUtc)
        {
            DateTime day = GetCmeTradingDayLabelCme(nowUtc);
        ...
        private void EmitControlEvent(string type, string reason)
        {
            ...
            // var json = $"{{\"tag\":\"CONTROL\",\"inst\":\"{Instrument.FullName}\",\"type\":\"{type}\",\"reason\":\"{reason}\",\"global_seq\":{seq},\"ts\":\"{DateTime.UtcNow:O}\"}}";
        ...
        private static string BuildFusedSessionStart()
        {
            return string.Format(
              "{{\"inst\":\"CONTROL\",\"tag\":\"CONTROL\",\"type\":\"fused_session_start\",\"ts\":\"{0:O}\",\"instruments\":[\"ES\",\"NQ\"],\"recorder_version\":\"{1}\"}}",
              NextGlobalUtc(), VERSION);
        }
            ...
            else if (State == State.Realtime)
            {
                ...
                lock (fusedSync)
                {
                    if (WriteFused)
                    {
                        ...
                        fusedFirstWriteTs = DateTime.UtcNow;
                        ...
                        while (!d.WriteCts.Token.IsCancellationRequested)
                        {
                            ...
                            while (d.SnapshotQueue.TryDequeue(out p))
                            {
                                try
                                {
                                    ...
                                    if (d.UnflushedRows >= 2000 || (DateTime.UtcNow - d.LastFlushUtc).TotalMilliseconds >= FlushMs)
                                    {
                                        d.Writer.Flush();
                                        d.LastFlushUtc = DateTime.UtcNow;
                                        d.UnflushedRows = 0;
                                    }
            ...
            else if (State == State.Terminated)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        string sessionDate = DateTime.UtcNow.ToString("yyyyMMdd", CultureInfo.InvariantCulture);
        ...
        private void EnsureWriter(DomState d)
        {
            ...
            string today = GetCmeTradingDayKey(NextGlobalUtc());
            d.SessionDate = today;
            d.Date = today;
            d.LastFlushUtc = DateTime.UtcNow;
            d.DensityResetTs = DateTime.UtcNow;
        ...
        protected override void OnMarketDepth(MarketDepthEventArgs e)
        {
            try
            {
                ...
                lock (d.Sync)
                {
                    ...
                    // [FIX 4] RELAXED TIME REGRESSION GUARD (Non-Colo VPS)
                    if (d.LastDepthUpdateExchTs > DateTime.MinValue && e.Time < d.LastDepthUpdateExchTs.AddMilliseconds(-120))
                    {
                        EmitRow(d, e.Time, Tags.DOM, 0, e.Price, LiqEvents.NONE, DateTime.MinValue, CausalLabels.NONE, force: true, forceReason: Reasons.TIME_REGRESSION);
                        return;
                    }
                    ...
                    // HARD WARMUP EXIT (TIME + STABILITY)
                    if (warmup)
                    {
                        double stableMs = (DateTime.UtcNow - d.LastTopChangeUtc).TotalMilliseconds;
                        var elapsedMin = (DateTime.UtcNow - sessionStartUtc).TotalMinutes;

                        if (elapsedMin >= 15 || (stableMs >= 300 && d.Seq >= 500))
                        {
                            warmup = false;

                            EmitControlEvent("warmup_exit",
                                elapsedMin >= 15 ? "time" : "stability");
                        }
                    }
                    ...
                    if (EnableSpoof)
                        UpdateSpoof(d, isBidSide: isBid, price: e.Price, delta: delta, bestBid: bestBid, bestAsk: bestAsk);

                    if (EnableIceberg)
                        UpdateIcebergFromDepth(d, e.MarketDataType, e.Price, oldVol, newVol);
                    ...
                    {
                        d.OfiCnt = 0; d.OfiMean = 0; d.OfiVar = 0; d.OfiStatResetTs = DateTime.UtcNow;
                    }
                    ...
                    lock (d.HistoryLock)
                    {
                        ...
                        {
                            Seq = d.Seq, ExchTs = e.Time, HostUtc = DateTime.UtcNow,
                    ...
                    if (UseSessionFilter && !sessionIterators[idx].IsInSession(Core.Globals.Now, true, true))
                    {
                    ...
                    if (!d.InBurst)
                    {
                        UpdateFeatureState(d, DateTime.UtcNow, bestBid, bestAsk, tB, tA, sB, sA, currentOfi, currentDepthOfi);
                    }
                    else
                    {
                        d.RvVarEma = 0;
                    ...
                    EmitRow(d, e.Time, Tags.DOM, 0, e.Price, liqEvent, DateTime.MinValue, CausalLabels.NONE, forceSession || forceGapFill, forceReason, liqSide, liqLevel, delta);
                }
            }
            catch (Exception ex)
            {
        ...
        protected override void OnMarketData(MarketDataEventArgs e)
        {
            ...
            lock (d.Sync)
            {
                ...
                d.TradeBuffer.AddLast(new QueuedTrade
                {
                    ExchTs = e.Time,
        ...
        private void ProcessPendingTrades(int idx)
        {
            ...
            while(node != null)
            {
                ...
                if (causalFound || isTimeout)
                {
                    ...
                    EmitRow(d, trade.ExchTs, Tags.TRADE, trade.Volume, trade.Price, LiqEvents.NONE, DateTime.MinValue, label, trainEligibleOverride: trade.TrainEligible);
                    removeCount++;
                }
                else
                {
        ...
        private static void FusedWriterLoop()
        {
            ...
            while (true)
            {
                try
                {
                    // [KERNEL] Deterministic Consumer
                    // No inference, no global_seq checks, no breaks.
                    // Writer advances iff slot contains exact seq.

                    if (fusedWriterCts.IsCancellationRequested) break;
                    ...
                    if (entry.Seq != fusedReadSeq)
                    {
                        ...
                        // Keep flush alive during spins to prevent buffer lockup
                        long now = Stopwatch.GetTimestamp();
                    ...
                    // Reset spin logic after work
                    spin.Reset();

                    // Optional: Flush on activity (less frequent)
                    long now2 = Stopwatch.GetTimestamp();
                    fusedLastWriteTicks = now2;
                    fusedLastHeartbeatTicks = now2; // Keep alive

                    if (now2 - lastDiskFlushTicks > flushIntervalTicks)
                    {
                        if (fusedWriter != null) fusedWriter.Flush();
                        lastDiskFlushTicks = now2;
                        OpenFusedFile();
                    }
                }
                catch (Exception ex)
        ...
        private void HeartbeatLoop()
        {
            while (!heartbeatCts.Token.IsCancellationRequested)
            {
                if (dom != null)
                {
                    ...
                    {
                        if (d == null || !d.IsMonitoring) continue;
                        if ((DateTime.UtcNow - d.LastHeartbeatUtc).TotalSeconds >= HeartbeatSec && HeartbeatSec > 0)
                        {
                            d.LastHeartbeatUtc = DateTime.UtcNow;
        ...
        // EMIT ROW
        // ==========================================================
        private void EmitRow(DomState d, DateTime eventTime, byte tag, long tradeVol, double eventPrice, byte liquidityEvent, DateTime? arrivalTime, byte tradeCausalityLabel = CausalLabels.NONE, bool force = false, byte forceReason = Reasons.NONE, int liqSide = 0, int liqLevel = -1, long liqDelta = 0, bool? trainEligibleOverride = null)
        {
            // --------------------------------------------------------------------------------
            ...
            // [FIX] Use the helper to guarantee monotonic time for ALL row types (DOM, TRADE, REPAIR)
            ...
            if (tag == Tags.TRADE && !d.IsDomBootstrapped)
            {
                ...
                if (d.LastDepthUpdateExchTs > DateTime.MinValue)
                {
                    EmitRow(d, d.LastDepthUpdateExchTs, Tags.DOM, 0, 0, LiqEvents.NONE, DateTime.MinValue, force: true, forceReason: Reasons.PRE_TRADE_SYNC);
                    if (!d.IsDomBootstrapped) {
                         d.DropsStructural++; d.StructuralDrops1s++; d.RowsDropped++; return;
                    }
                }
                else
                {
            ...
            {
                repairType = RepairTypes.STRUCTURAL;
                repairReason = RepairReasons.REWRITE_FIELDS;
            }
            ...
            double stableMs = (nowUtc - sessionStartUtc).TotalMilliseconds;
            if (stableMs < 0) stableMs = 0;

            // enforce monotonic stable clock (audit will stop failing)
            if (stableMs < d.LastStableMs)
            {
                d.RowsDropped++;
                return;
            }
        ...
        private void UpdateSpoof(DomState d, bool isBidSide, double price, long delta, double bestBid, double bestAsk)
        {
        ...
        private void UpdateIcebergFromDepth(DomState d, MarketDataType mdt, double price, long oldVol, long newVol)
        {
            ...
            long visibleChange = oldVol - newVol;
            bool isRelevantSide = (d.Pending.Side > 0 && mdt == MarketDataType.Ask) || (d.Pending.Side < 0 && mdt == MarketDataType.Bid);
        ...
        private void WriteLatencyStats(DomState d)
        {
            ...
            try
            {
                ...
                string json = $"{{\"tag\":\"metadata\",\"type\":\"latency_stats\",\"ts\":\"{DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.ffffff", CultureInfo.InvariantCulture)}\",\"latency_ms\":{{\"p50\":{p50},\"p95\":{p95},\"max\":{d.LatencyMax},\"count\":{d.LatencyCount},\"source\":\"non_colocated\"}}}}\n";
                d.Writer.Write(json);
                d.Writer.Flush();
            }
            catch {}
